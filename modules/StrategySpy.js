...
var Strategy = function(app) {
	...
	var _clearBatchesBeforeActual = 3;
	...
	this.clearPreviousData = function(res, config, done) {
	...
	this.sendBacktestOrders = function(config, done) {
		...
		console.log("Current state: Equity: ", config.currentState.current_capital, " Unused: ", config.currentState.unused_capital, " Free pieces: ", config.currentState.free_pieces);
		...
		async.series([
			...
			function(done) {
				...
				async.each(config.openPositions, function(pos, done) {
					console.log("OPEN: "+ pos.amount + "x "+pos.ticker+ " for "+ pos.pieces * pos.amount);
					DB.insert("positions", {
	...
	this.init = function(config, done) {
		...
		async.waterfall([
			...
			self.clearPreviousData.bind(null, config),
	...
	this.appendActualPrices = function(config, done) {
		// console.log("Appending actual prices to historical data");

		for(var symbol in config.actual) {
			var item = config.actual[symbol];

			if(!config.data[symbol]) {
				console.log("Ticker(",symbol,") does not exists in data feed!");
				continue;
			}
	...
	this.saveIndicators = function(config, done) {
		...
		DB.insertValues("indicators (ticker, price, sma5, sma100, sma200, rsi14, import_id)", buffer, function(err, res) {
			done(err, config);
		});
	};
	...
	this.getStocksForBuy = function(config) {
		...
		for(var ticker in config.indicators) {
			...
			if( item.rsi <= 40 && item.price > item.sma200 && item.price <= item.sma100) {
				if(config.closePositions[ticker])
					delete config.closePositions[ticker];
				stocks.push(item);
			}
		};
	...
	this.filterBuyStocks = function(config, done) {
		...
		stockForBuy = self.getStocksForBuy(config);
		...
		for(var i = 0; i < stockForBuy.length; i++) {
			...
			if(config.newState.unused_capital < 0 )
				throw "BAD STATE - unused capital < 0";
		}
	...
	this.filterSellStocks = function(config, done) {
		...
		async.each(Object.keys(config.positionsAggregated), function(ticker, done) {
			...
			self.isLastPriceEntry(ticker, config.date, function(isLast) {
		        ...
		        if( ( indicators.rsi > 70 && indicators.sma5 > indicators.price ) || isLast) { // is openned and sma5 > actual price
