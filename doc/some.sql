...
-- compare ALL 
SELECT cap90, cap90_2, capEQ, capA, e1.date FROM (SELECT DATE(date) as date, capital as capEQ FROM `equity_history_90_boost`) as e1 JOIN (SELECT DATE(date) as date, capital as cap90 FROM `equity_history_90`) as e3 JOIN (SELECT DATE(date) as date, capital as cap90_2 FROM `equity_history_90_2`) as e4 JOIN (SELECT DATE(date) as date, capital - 235 as capA FROM equity_history_stock_picking) as e2 ON e1.date = e2.date AND e2.date = e3.date AND e3.date = e4.date
